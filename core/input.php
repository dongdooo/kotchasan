...
class Input
{
	...
	/**
	 * ตรวจสอบชื่อโมดูลที่เรียก
	 * ชื่อโมดูล ภาษาอังกฤษ เท่านั้น
	 *
	 * @param array $var เช่น $_GET หรือ $_POST
	 * @return string
	 */
	public static function getModule($var)
	{
		if (isset($var['module'])) {
			$module = strtolower($var['module']);
	...
	/**
	 * ตรวจสอบชื่อเมธอดที่เรียก
	 * ได้แก่ model, controller และ view
	 *
	 * @param array $var เช่น $_GET หรือ $_POST
	 * @return string
	 */
	public static function getMethod($var)
	{
		if (isset($var['method'])) {
			$method = strtolower($var['method']);
			if (in_array($method, array('model', 'controller', 'view'))) {
				return $method;
			}
		}
	}
	...
	/**
	 * ตรวจสอบไฟล์ที่เรียก
	 * ชื่อไฟล์ ภาษาอังกฤษตัวพิมพ์เล็ก ตัวเลข และ _ เท่านั้น
	 *
	 * @param array $var เช่น $_GET หรือ $_POST
	 * @return string
	 */
	public static function getPage($var)
	{
		if (isset($var['page'])) {
			$page = strtolower($var['page']);
	...
	/**
	 * ตรวจสอบฟังก์ชั่นที่เรียก
	 * ชื่อฟังก์ชั่น ภาษาอังกฤษ ตัวเลข และ _ เท่านัน
	 *
	 * @param array $var เช่น $_GET หรือ $_POST
	 * @return string
	 */
	public static function getFunction($var)
	{
		if (isset($var['function']) && preg_match('/^[a-zA-Z0-9_]+$/', $var['function'])) {
			return $var['function'];
		}
	}
