...
class View extends \View
{
	...
	{
		$categories = \Board\Categories\Model::get($index);
		// รายการ
		$listitem = \Template::create($index->owner, $index->module, 'categoryitem');
		foreach ($categories as $item) {
			if (!empty($item->icon) && is_file(ROOT_PATH.\Kotchasan::$data_folder.'board/'.$item->icon)) {
				$icon = WEB_URL.\Kotchasan::$data_folder.'board/'.$icon;
			} else {
