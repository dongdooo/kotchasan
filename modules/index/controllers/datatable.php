<?php
/**
 * @filesource index/controllers/datatable.php
...
class Controller extends \Controller
{
	...
	public function index()
	{
		...
		// โหลดเมนู
		foreach (self::$menu->getMenus() as $item) {
			$module = $item->module;
		...
		$home = self::$menu->homeMenu();
		$home->canonical = WEB_URL.'index.php';
		// breadcrumb หน้า home
		self::$view->addBreadcrumb($home->canonical, $home->menu_text, $home->menu_tooltip, 'icon-home');
		// ตรวจสอบหน้าที่เรียก
		...
		// หน้า home
		if (empty($module) || $home->module_id == $module->module_id) {
			$page->canonical = $home->canonical;
		}
		// เมนู
		self::$view->add(self::$menu->render());
		if (empty($page)) {
			// ไม่พบหน้าที่เรียก
		...
		// โหลด datatable
		$datatable = $this->createClass('Index\Datatable\View');
		// เนื้อหา
		self::$view->add(array(
			// content
			'CONTENT' => $datatable->index(),
			// กรอบ login
			'LOGIN' => 'Login',
			// title
