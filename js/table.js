...
GTable.prototype = {
	initialize: function (id, o) {
		...
		if (this.table) {
			forEach($G(this.table).elements('th'), function () {
			...
			var doAction = function () {
				var action = '',
					cs = new Array(),
					chk = /check_[0-9]+/;
				forEach(temp.table.elements('a'), function () {
			...
			forEach(this.table.elements('label'), function () {
			...
			if (this.options.action) {
				...
				forEach(this.table.elements('a'), function () {
					var id = this.id;
					if (id && !move.test(id)) {
			...
			if (this.options.dragColumn > -1) {
				new GSortTable(this.table, {
					'endDrag': function () {
						var trs = new Array();
						forEach(temp.table.elements('tr'), function () {
							if (this.id) {
	...
	inintTR: function (el) {
		var row = 0,
			temp = this;
		forEach($G(el).elements('tr'), function () {
			if (this.parentNode.tagName.toLowerCase() == 'tbody') {
				this.id = temp.table.id + '_' + row;
				forEach($G(this).elements('input'), function () {
	...
	inint: function (el) {
		...
		var aClick = function () {
			...
			if (c == 'icon-plus') {
				...
				var tr = $G(this.parentNode.parentNode.parentNode);
				var tbody = tr.parentNode;
				if (tbody.elements('tr').length > 1 && confirm(trans('You want to delete ?'))) {
					tr.remove();
					temp.inintTR(tbody);
				}
		...
		var saClick = function () {
			...
			forEach(el.elements('a'), function () {
		...
		forEach($G(el).elements('a'), function () {
	...
	setSort: function (sort, patt) {
		var hs;
		forEach(this.table.elements('th'), function () {
	...
	redirect: function () {
		...
		if (this.sort) {
			...
			us.push('sort_type=' + (this.sort_type == 'asc' ? 'asc' : 'desc'));
		}
		if (this.search) {
